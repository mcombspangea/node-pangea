...
const embargoConfig = new PangeaConfig({ baseDomain: `${pangeaCsp  }.pangea.cloud`, configId: embargoConfigId });
const embargo = new EmbargoService(token, embargoConfig);

const auditConfig = new PangeaConfig({ baseDomain: `${pangeaCsp  }.pangea.cloud`, configId: auditConfigId });
const audit = new AuditService(token, auditConfig);

const redactConfig = new PangeaConfig({ baseDomain: `${pangeaCsp  }.pangea.cloud`, configId: redactConfigId });
...
class App {
  ...
  async fetchEmployeeRecord(user, email) {
    ...
    if (logResponse.code === 200) {
      ...
      console.log("[App.fetchEmployeeRecord] Audit log error: ", logResponse.code, logResponse.result);
    }
  ...
  async updateEmployee(user, data) {
    ...
    if (oldEmp) {
      const newEmp = Object.assign({}, oldEmp); // keeping old_emp around for audit logging
      ...
      return [ ret ? 200: 500, ret ? "Successfully updated employee record" : "Database update error"]
    } 
